...
Excel 合并、PDF 排序集中到同一个 CLI 与 PySide6 桌面界面中。批量重命名、内容替换、
发票识别、PDF 排序、Excel 合并、建文件夹和生成 PDF 的 CLI 默认只预览，显式传入
...
> 发票识别、PDF 排序和 Excel 合并可在 Windows、macOS 和 Linux 上使用。
...
所有文件写入命令默认预览，添加 `--yes` 才执行。`mkdir` 和 `pdf` 也遵循此规则；
已有自动化脚本如需实际创建目录或生成 PDF，须显式补上 `--yes`。
PDF 默认只显示源文件与转换选项，不启动 Office 或生成输出，不代表转换已成功。

正常预览和全部成功执行返回 0。确认执行时，只要有失败（包括部分失败、重命名冲突、
历史保存失败）就返回非零，已成功的输出和失败路径仍在终端显示，避免重试时误认为没有写入。
取消、输入错误、资源释放失败同样不返回成功；业务失败通常为 1，Typer 参数错误为 2。
`mkdir --on-conflict` 只接受 `skip` 或 `merge`。PDF 和内容替换服务在异常与取消时也会释放资源，
收尾失败不会覆盖原处理错误。预览显示的冲突/匹配信息保持只读计划语义，不作为执行失败。

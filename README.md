...
推送 `v*` tag 后,`.github/workflows/release.yml` 自动执行三个 job:

1. **build**(Windows):Nuitka 打包,输出便携 zip + 校验和。
2. **release**(Ubuntu):创建 GitHub Release 并附带 zip + 校验和。
3. **sync**(Ubuntu):把 `main` 分支 + 当前 tag 同步到 gitee / cnb 镜像仓,在 Gitee 创建 Release 并上传产物,并清理 GitHub + Gitee 的旧 Release(保留最近 5 个)。任一镜像失败不影响主发版(`continue-on-error`)。

#### 镜像同步 Secrets

发版后 sync job 自动同步代码到 gitee / cnb 镜像仓,并在 Gitee 创建 Release、清理旧 Release。
需在 GitHub 仓库 **Settings → Secrets and variables → Actions** 添加:

| Secret | 用途 | 获取方式 |
|--------|------|----------|
| `GITEE_TOKEN` | 推送 gitee + 创建 Gitee Release | Gitee → 个人设置 → 私人令牌(需 repo 权限) |
| `CNB_TOKEN` | 推送 cnb(仅同步代码) | cnb.cool → 个人设置 → 访问令牌 |

> `GH_TOKEN` 使用 Actions 内置 `GITHUB_TOKEN`,无需手动添加。
> 任一 token 缺失,对应平台的同步步骤会跳过(不报错),主发版不受影响。
...
- **更新源**:Gitee + GitHub 并发,先到先得(国内访问 Gitee 通常更快)。

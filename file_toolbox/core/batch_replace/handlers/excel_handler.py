...
class ExcelHandler(LoggableMixin):
    ...
    def replace_single_file(self, file_path: Path, operations: list[dict]) -> int:
        """
        ...
        try:
            ...
            for operation in operations:
                try:
                    count = self._execute_operation(wb, operation)
            ...
            self.logger.error(f"Excel文档替换失败: {file_path} - {e}")

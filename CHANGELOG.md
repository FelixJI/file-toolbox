...
- **gui:** 延迟功能页重依赖消除首切卡顿 (#126) (4582f91)
- **pdf:** 修复引擎检测回显并消除跨进程冗余验证 (#125) (251fc16)
...
- **excel:** 撤回失败源并保护工作簿收尾 (9035509)
- **gui:** 等待真实线程结束后安全关闭窗口 (#115) (4116b29)
- **gui:** 保留合并结果并隔离附属保存告警 (#114) (d9acdba)
- **output:** 排他提交 Excel 与 PDF 输出并清理失败暂存 (#113) (f1fa26b)

## 0.3.2

### Features

- **plan-schedule:** 新增计划排布功能（按月排布、周末标记、逐日并行数） (#95) (b91ecbd)
- **ai-flow:** 升级原生协议并迁移本机产物 (#86) (7e6085c)
- **ai-flow:** 升级 Issue 驱动的本地调度流程 (#85) (67dcb70)
- **ai-flow:** 集成 balanced 接入配置 (#76) (79094f4)

### Bug Fixes

- **replace:** 阻止备份失败文件进入写入批次 (#105) (5e33535)
- **pdf-sort:** 保留取消前的部分结果与历史 (#104) (8fbbf3b)
- **attendance:** 原子拒绝覆盖并发出现的输出 (#103) (15db4f4)
- **gui:** 保留页面懒加载失败后的重试入口 (#101) (ff27286)
- **cli:** 统一执行确认失败退出与资源释放 (#98) (6bf49bf)
- **cli:** 保留操作文本并按字段归一化参数 (#96) (bca0345)
- **updater:** 同步取消与应用更新的请求边界 (#94) (a755cb7)
- **rename:** 统一真实结果与可重试安全撤销 (#93) (ec9e6e6)
- **storage:** 修复 history/settings 跨进程并发与损坏一致性 (#84) (80c4a22)
- **ai-flow:** 按推送目标限定历史产物检查 (#91) (0795dde)
- **ai-flow:** 统一同主体连续执行与人工交接边界 (#89) (2de60ac)

## 0.3.1

### Features

- **gui:** 优化关于页更新交互与更新进度/日志展示并并发探测加速镜像 (#74) (f71ddaf)
...
- **release:** 规范便携包文件名 (#56) (166b436)

### Performance

- **gui:** 加快启动并修复窗口高度被钉死 (#57) (43a7c73)

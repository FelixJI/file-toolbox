...
读取项目适用 AGENTS/override、`.ai-flow/AGENTS.md`、`project.json`、本机 `local.json`、指定 Goal Issue/主 Issue、关联 Task Issues、相关代码/测试和未合并 PR。Issue 是任务合同；不要重复生成一份长期人工计划。不存在的远端事实不能编造。

先核对 `.ai-flow/docs/RUNNER.md`。默认 balanced + native-event-driven；复杂工作 Codex，边界清楚的工作 pi+已配置GLM。用户不再分别启动 pi，也不逐项复制施工提示词。
...
3. 使用干净、专用 Git worktree/分支。不得清理或 stash 用户未提交内容；确认没有其他写入者。新 worktree 需配套的 ignored local.json；本机配置不会由 Git 自动带入。
4. 一次调用：`uv run --frozen python .ai-flow/scripts/flow.py start --intake .ai-flow/runtime/intake.md --detach`。旧版 `--plan` 仅保留兼容。本仓所有 Python 入口统一使用锁定的 uv 环境。仅当本机 local.json 已记录当前原生协议在该宿主跨回合存活的实测证据时使用 `--detach`；否则在可信终端省略该参数以前台运行。不要在控制器子会话中再次调用本入口。
5. 启动输出 SUBMITTED 不是 RUNNING。仅允许一次启动确认读取 `status`/日志；不能反复轮询。确认不足则如实说明“已提交，运行存活待验证”。
6. 当前桌面会话结束本轮；把工作区写入权交给 runner。不要与 runner 同时改代码、不要启动 `codex exec resume --last`、不要每隔几秒读取日志。coordinator 会在完整原生完成事件后恢复 Controller turn，实施与独立审阅使用隔离 thread。
